...
Fix: split connection creation (`_make_conn`, still `@st.cache_resource`) from a wrapper (`_get_live_conn`) that checks `conn.closed` before handing it out and clears the cache to force a fresh connection if so -- catches client-visible closure proactively. For the server-closed-but-client-doesn't-know-yet case, added a dedicated `except psycopg2.InterfaceError` branch *before* the general `except psycopg2.Error` one, which clears the cached connection and calls `st.rerun()` instead of attempting `.rollback()` on a connection already confirmed dead. Take-away: a cached long-lived resource needs its failure mode designed in from the start, not just its happy path -- the existing rollback-and-reraise handler looked like it already covered "connection problems," but it silently assumed the connection was merely in a bad transaction state, never that it might not exist anymore.
