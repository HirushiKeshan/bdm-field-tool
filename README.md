...
- **A native app / installed mobile client.** This is a website, not an app-store install — deliberately, since it means zero install friction and instant updates for 12 reps. A phone's "Add to Home Screen" gets most of the same feel without the App Store review cycle a real native app would need.
...
4. **GPS is captured but never used to identify which outlet was visited** — only to flag physically impossible patterns. The app captures the BDM's real device location via the browser's geolocation API (`logic/confidence.py::flag_location_mismatch`) at check-in, and flags it only if the phone is more than 500m from the outlet's registered address — generous on purpose, since ordinary phone GPS error is 10-50m and the flag exists to catch "checked in from home," not to referee two counters a few metres apart. A reading is skipped entirely, not flagged, if the device itself reports over 1km of accuracy (no real fix, e.g. no signal indoors) rather than treating noise as evidence. Against the 3-month historical log, this class of check (plus daily-volume and pacing checks) flags 715 of 5,904 visits (12.1%), split roughly evenly across implausible daily visit counts, impossibly tight pacing between different outlets, and visits outside the BDM's assigned territory. That number is itself the strongest evidence for why the old log couldn't be trusted, and it's surfaced directly in the manager view's Log Integrity section rather than buried.
