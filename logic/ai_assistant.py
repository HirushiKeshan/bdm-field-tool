"""
Groq-backed "ask a question" box for the Insights page. The model is
never given database access and never asked to calculate anything on
its own -- it only ever sees the exact same JSON snapshot the Insights
page already renders (see db.manager_queries.insights_summary) and is
told, explicitly, to answer from that data alone or say the data
doesn't cover it. This app's whole pitch is "don't trust a number you
can't verify" -- an assistant that could invent a number would break
that on the first question asked of it.
"""
import json
import os
